...
func _get_input() -> void:
	...
	if _is_snapped:
		...
		if distance < UNSNAP_DISTANCE and distance > SNAP_PADDING:
			...
			$Sprite2D.scale.x = 0.5 - (position.distance_to(_last_position) / 200)
			$Weapon/Sprite.rotation = -10 - (position.distance_to(_last_position) / 100)
			$Weapon/Sprite.scale.x = 1 - (position.distance_to(_last_position) / 200)
